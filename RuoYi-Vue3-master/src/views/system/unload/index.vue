<template>
  <div class="app-container">
    <el-form :model="queryParams" ref="queryForm" size="small" :inline="true" v-show="showSearch" label-width="68px">
      <el-form-item label="船名称" prop="boatname">
        <el-input
    ...
    <el-row :gutter="10" class="mb8">
      ...
      <el-col :span="1.5">
        <el-button
          ...
          :disabled="multiple"
          @click="handleDelete"
          v-hasPermi="['system:unload:remove']"
        >删除</el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button
          ...
          size="mini"
          @click="handleExport"
          v-hasPermi="['system:unload:export']"
        >导出</el-button>
      </el-col>
      <right-toolbar :showSearch.sync="showSearch" @queryTable="getList"></right-toolbar>
    </el-row>

    <el-table v-loading="loading" :data="unloadList" @selection-change="handleSelectionChange">
      <el-table-column type="selection" width="55" align="center" />
      <el-table-column label="船名称" align="center" prop="boatname" />
    ...
    <el-dialog :title="title" v-model="open" width="500px" append-to-body>
      <el-form ref="form" :model="form" :rules="rules" label-width="80px">
        <el-form-item label="船名称" prop="boatname">
          <el-input v-model="form.boatname" placeholder="请输入船名称" />
        </el-form-item>
        <el-form-item label="作业开始时间" prop="begining">
...
<script>
import { listUnload, getUnload, delUnload, addUnload, updateUnload } from "@/api/system/unload";
...
export default {
  ...
  methods: {
    ...
    getList() {
      this.loading = true;
      listUnload(this.queryParams).then(response => {
        this.unloadList = response.rows;
      ...
      this.$refs["form"].validate(valid => {
        if (valid) {
          if (this.form.id != null) {
            updateUnload(this.form).then(response => {
              this.$modal.msgSuccess("修改成功");
              this.open = false;
              this.getList();
            });
          } else {
            addUnload(this.form).then(response => {
    ...
    handleDelete(row) {
      ...
      this.$modal.confirm('是否确认删除卸货表编号为"' + ids + '"的数据项？').then(function() {
        return delUnload(ids);
      }).then(() => {
        this.getList();
    ...
    /** 导出按钮操作 */
    handleExport() {
      this.download('system/unload/export', {
        ...this.queryParams
      }, `unload_${new Date().getTime()}.xlsx`)
    }
  }
};
</script>
