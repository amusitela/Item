    ...
    <el-row :gutter="10" class="mb8">
      <el-col :span="1.5">
        <el-button
          ...
          size="mini"
          @click="handleAdd"
          v-hasPermi="['clean:insertsWork:add']"
        >新增</el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button
          ...
          :disabled="single"
          @click="handleUpdate"
          v-hasPermi="['clean:insertsWork:edit']"
        >修改</el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button
          ...
          :disabled="multiple"
          @click="handleDelete"
          v-hasPermi="['clean:insertsWork:remove']"
        >删除</el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button
          ...
          size="mini"
          @click="handleExport"
          v-hasPermi="['clean:insertsWork:export']"
        >导出</el-button>
      </el-col>
      <right-toolbar :showSearch.sync="showSearch" @queryTable="getList"></right-toolbar>
    </el-row>

    <el-table v-loading="loading" :data="insertsWorkList" @selection-change="handleSelectionChange">
      ...
      <el-table-column label="作业开始时间" align="center" prop="ws" width="180" />
      <el-table-column label="作业结束时间" align="center" prop="we" width="180" />

      <el-table-column label="始发时间" align="center" prop="start" width="180" />

      <el-table-column label="到达时间" align="center" prop="end" width="180" />
...
<script>
import { listInsertsWork, getInsertsWork, delInsertsWork, addInsertsWork, updateInsertsWork } from "@/api/clean/insertsWork";
...
export default {
  ...
  methods: {
    ...
    getList() {
      this.loading = true;
      listInsertsWork(this.queryParams).then(response => {
        this.insertsWorkList = response.rows;
    ...
    this.flag=0;
      ...
      getInsertsWork(num).then(response => {
        this.form = response.data;
        this.open = true;
        this.title = "修改userWork";
      });
    },
    /** 提交按钮 */
    submitForm() {
      this.$refs["form"].validate(valid => {
        if (valid) {
          if (this.falg == 0) {
            updateInsertsWork(this.form).then(response => {
              this.$modal.msgSuccess("修改成功");
              this.open = false;
              this.getList();
            });
          } else {
            addInsertsWork(this.form).then(response => {
    ...
    handleDelete(row) {
      ...
      this.$modal.confirm('是否确认删除userWork编号为"' + nums + '"的数据项？').then(function() {
        return delInsertsWork(nums);
      }).then(() => {
        this.getList();
    ...
    /** 导出按钮操作 */
    handleExport() {
      this.download('clean/insertsWork/export', {
        ...this.queryParams
      }, `insertsWork_${new Date().getTime()}.xlsx`)
    }
  }
};
</script>
